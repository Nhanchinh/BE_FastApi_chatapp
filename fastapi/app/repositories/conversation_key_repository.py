...
class ConversationKeyRepository:
    ...
    async def store_key(self, conversation_id: str, user_id: str, encrypted_session_key: str) -> str:
        """
    ...
    async def get_key(self, conversation_id: str, user_id: str) -> Optional[dict]:
        ...
        doc = await self.collection.find_one({"conversation_id": conversation_id, "user_id": user_id})
        if doc:
            doc["_id"] = str(doc["_id"])
        return doc
